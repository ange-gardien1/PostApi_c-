...
public class AuthService : IAuthService
{
    ...
    public IEnumerable<User> SearchUsers(string text)
    {
        text = text.ToLower();
        return _context.Users.Where(u => u.Username.ToLower().StartsWith(text)
            || u.FirstName.ToLower().StartsWith(text)
            || u.LastName.ToLower().StartsWith(text));
    }
}